...
\citet*{HN94a} schlagen deshalb die Benutzung eines speziellen Dominanzschemas vor, 
...
kann man feststellen, dass dieses Beispiel unter den normalerweise geltenden Annahmen nicht string vacous erklärt werden kann.
\eal
\ex[]{
...
in Sätzen direkt verwendet werden kann. Dass dem nicht so ist, zeigt Haiders Beispiel
(\ref{ex-dass-wer-mit-was-wen-traktiert-hat}). G.\,\citet[80]{GMueller2014a-u} erlaubt \emph{tucking
  in}\is{tucking in} \citep[99]{Richards2001a-u}. Mit dieser Variante von Merge kann er folgende Ableitung aus (\mex{0}) bekommen:
\ea[]{
[[wen]$_i$ [mit was]$_j$ [\_$_i$ \_$_j$ traktiert]] hat
}
\z
\emph{Tucking in} bedeutet \emph{hineinstopfen}. Im Fall von (\mex{0}) wird \emph{mit was} nicht
ganz nach links bewegt, sondern zwischen \emph{wen} und \emph{traktiert} eingesetzt. Das ist
unüblich und verletzt die \emph{No Tampering Condition}\is{No Tampering Condition}, die normalerweise angenommen wird: Was
einmal zusammengebaut ist, wird nicht wieder auseinandergenommen, um etwas in die Mitte einzusetzen. \citet{CS2016a-u} stellen eine
Formalisierung Minimalistischer Syntax vor und halten explizit fest, dass tucking in in ihrem
System von der No Tampering Condition ausgeschlossen wird (Fn.\,9). Vielleicht kann G.\ Müller
seinen Ansatz irgendwie formalisieren, trivial ist das aber sicherlich nicht. Der hier vorgestellte
Ansatz mit Argumentkomposition ist dagegen relativ einfach und wird in verschiedenen Frameworks verwendet
(GPSG: \citealt[878]{Johnson86a}; HPSG: \citealt{HN94a,Kiss95a,Mueller96a,Meurers2000b};
Minimalismus: \citealt[618--643]{Sternefeld2006a-u}). 

Letztendlich ist tucking in eine Hilfskonstruktion, für die es keine Evidenz gibt. Bei normalem
Scrambling braucht man es nicht. Ansätze, die ohne auskommen, sind vorzuziehen.
\is{Verbalkomplex|)}
